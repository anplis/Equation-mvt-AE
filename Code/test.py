...
def opti_cst(f):
    ...
    CSTES = [c for c in f.atoms(sy.Float)]
    if CSTES:
        c = rd.choice(CSTES)
...
def evolution_one_var(i_var):
    ...
    while len(population)<n_voisin_S :  # Continue jusqu'à ce que la population atteigne la taille souhaitée
        ...
        f_no_nb = no_nb(f)
        if f_no_nb not in memory:
            add_memory(memory, f_no_nb)
            fit = fitness(f, i_var)
    ...
    while gen<max_gen and ecart(population[0][1],i_var)>eps:  # Continue tant que le nombre de générations est inférieur à max_gen et que l'écart du meilleur individu est supérieur à eps
        ...
        for f_fit in f_fits:
            ...
            for _ in range(n_voisin_S):
                ...
                new_f_no_nb = no_nb(f)
                while new_f_no_nb in memory and m<50:
                    if m>20:
                        n_mut += 1
                    m += 1
                ...
                if validate_function(new_f):  # Vérifie si la nouvelle fonction est valide
                    add_memory(memory, new_f_no_nb)  # Ajoute la nouvelle fonction à la mémoire
            ...
            # - numériques(change csts)
            for _ in range(n_voisin_N):
                new_f = opti_cst(f_fit[0])
                if validate_function(new_f):  # Vérifie si la nouvelle fonction est valide
                    fit = fitness(new_f, i_var)
                    if isinstance(fit, (int, sy.Float)) and abs(fit) < float('inf'):  # Vérifie si l'écart est un nombre réel
                        population = insert_sorted(population, (new_f, fit))  # Insère la nouvelle fonction dans la population triée        
